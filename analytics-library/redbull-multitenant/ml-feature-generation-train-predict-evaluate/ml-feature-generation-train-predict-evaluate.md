...
3. Open the menu by pressing the hamburger icon, then open the Notebooks option.

   ![pic1](images/start-notebooks.png)

4. Start the "Import Notebook" option.

   ![pic1](images/import-notebook.png)

5. Then select the redbull-notebook.json file from your file system.
...
6. Now click on the notebook to open it.

   ![pic1](images/click-notebook.png)

7. Disable the MEDIUM and HIGH connection methods.

   We do this to not overload the database when all workshop participants run Machine Learning at the same time.

   ![pic1](images/disable-medium-high.png)

8. Inspect the notebook
...
10. Running the notebook may take several minutes.
...
3. Now we configure how to train our model.
   ...
   Data Source: `WORKSHOP<NUMBER>.RACES_TRAIN` (choose your `WORKSHOP<NUMBER>` schema and the `RACES_TRAIN` table from the popup window). Remember that this table holds the races until the end of 2019.
...
4. Open the "Additional Settings" section, and select "Negative Mean Absolute Error". This is a measurement of how close the predicted values are to the actual values. The training process will try to minimize this value.
   ...
   **Keep the value of "Database Service Level" at "Low". This is important to not overload the database while all workshop participants are training Machine Learning models.**

   **Check the "Generalized Linear Model", keep the other algorithms unchecked.**
...
## **Task 3:** Add the generated features to the Races data set in Oracle Analytics Cloud
...
2. Now we have to make the newly generated features (columns) from the RACES table visible to Oracle Analytics Cloud.
...
## **Task 4:** Import the trained model into Oracle Analytics Cloud
...
## **Task 5:** Predict the scores for 2020 in Oracle Analytics Cloud
...
## **Task 6:** Evaluate the accuracy of the model in Oracle Analytics Cloud
...
## **Task 7:** Bonus exercise: Calculate the difference between predicted and actual scores
...
## **Task 8:** Bonus exercise: Predict the scores for the 2021 season
...
## **Task 9:** Conclusions and next steps
